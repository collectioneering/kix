...
internal class RunDbList : RunDbBase
{
    protected Option<string> AugmentOption;

    protected Option<string> OutputOption;

    public RunDbList() : this("db-list", "List archives in database.")
    {
    }

    public RunDbList(string name, string? description = null) : base(name, description)
    {
        AugmentOption = new Option<string>(new[] { "--augment" }, "Use base profile file for profile generated by --output.");
        AugmentOption.ArgumentHelpName = "file";
        AddOption(AugmentOption);
        OutputOption = new Option<string>(new[] { "-o", "--output" }, "Generate profile file.");
